...
ASTNode* parse_function(FILE *input, Token return_type, Token func_name) {
    ...
    Token possible_struct_token; (void)possible_struct_token; // suppress unused warning if any
    ...
    g_array_count = 0; // Reset per-function array table
    ...
    if (!consume(input, TOKEN_PARENTHESIS_OPEN)) {
        ...
        free_node(func_node); exit(EXIT_FAILURE);
    }
    ...
    while (!match(TOKEN_PARENTHESIS_CLOSE) && !match(TOKEN_EOF)) {
        if (match(TOKEN_KEYWORD)) {
            if (strcmp(current_token.value, "struct") == 0) {
                advance(input);
                if (match(TOKEN_IDENTIFIER)) { param_type = current_token; advance(input); }
                else { printf("Error (line %d): Expected struct name in parameter list\n", current_token.line); break; }
            } else { param_type = current_token; advance(input); }
            if (match(TOKEN_IDENTIFIER)) {
                param_name = current_token; advance(input);
                ...
                if (match(TOKEN_COMMA)) advance(input);
            } else { printf("Error (line %d): Expected parameter name\n", current_token.line); break; }
        } else { advance(input); }
    }

    if (!consume(input, TOKEN_PARENTHESIS_CLOSE)) { printf("Error (line %d): Expected ')' after parameter list\n", current_token.line); free_node(func_node); exit(EXIT_FAILURE);}    
    if (!consume(input, TOKEN_BRACE_OPEN)) { printf("Error (line %d): Expected '{' to start function body\n", current_token.line); free_node(func_node); exit(EXIT_FAILURE);}    

    int brace_depth = 1;
    while (brace_depth > 0 && !match(TOKEN_EOF)) {
        if (match(TOKEN_BRACE_OPEN)) { brace_depth++; advance(input); }
        else if (match(TOKEN_BRACE_CLOSE)) { brace_depth--; advance(input); }
        else { ASTNode* stmt = parse_statement(input); if (stmt) add_child(func_node, stmt); }
    }
